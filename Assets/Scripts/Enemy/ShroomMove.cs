...
public class ShroomMove : MonoBehaviour {
	...
	void Update () {
        ...
        if (getHit.getHit)
        {
            ...
            rigid.velocity = new Vector2(Mathf.Sign(player.transform.position.x - transform.position.x) * veloci.x, 0);
        }
        if (set)
        {
            if (rigid.velocity.y < 0 && animatorShroom.GetBool("fly"))
            {
	...
	void OnTriggerStay2D (Collider2D coll)
	{
		if (coll.gameObject.tag == "Player") {
			rigid.velocity = new Vector2 (Mathf.Sign(coll.gameObject.transform.position.x - transform.position.x) * veloci.x , 0);
		}
		if (coll.gameObject.tag == "Platform" && set ) {
			rigid.velocity = veloci;
		}
	}
}
