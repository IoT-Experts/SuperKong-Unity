...
public class PlayerController : MonoBehaviour
{
    ...
    */
	...
	void Update ()
	{
		...
		if (Mathf.Abs (rid.velocity.x) > 5 && grounded) {
	...
	void ControlUserInput ()
	{
		...
		if (!playerGetHit) {
			...
			if (!attackState.attacking && !ground_plaform_move)
				...
				rid.velocity = new Vector2 (0, rid.velocity.y);
	...
	void DiveControl ()
	{
		if (dive) {
			if ((transform.position.y - currentPosPlayer.y) < Parameters.maxSpeedInHeight / 2) {
				rid.velocity = Vector2.Lerp (rid.velocity, new Vector2 (rid.velocity.x, 7), Time.deltaTime * Parameters.SpeedDiveUp);
			} else {
				rid.velocity = Vector2.Lerp (rid.velocity, new Vector2 (rid.velocity.x, 7), Time.deltaTime * Parameters.SpeedDiveUp / 2);
			}
	...
	void MoveHorizontal ()
	{
		if (horizontal < 0 && transform.position.x - limitLeft < 0.1f) {
			...
			rid.velocity = Vector2.Lerp (rid.velocity, new Vector2 (horizontal * MaxSpeedPlayer, rid.velocity.y), Time.deltaTime * Parameters.accelSpeed);

		if (horizontal == 0)
			rid.velocity = Vector2.Lerp (rid.velocity, new Vector2 (horizontal * MaxSpeedPlayer, rid.velocity.y), Time.deltaTime * Parameters.bakeSpeed);

		if (Mathf.Abs (rid.velocity.x) > Parameters.maxSpeed) {
			rid.velocity = new Vector2 (Parameters.maxSpeed * horizontal, GetComponent<Rigidbody2D> ().velocity.y);
		}
	...
	void JumpInAir ()
	{     
		if (!grounded && Mathf.Abs (rid.velocity.x) > Parameters.maxSpeedInHeight)
			rid.velocity = new Vector2 (Parameters.maxSpeedInHeight * horizontal, rid.velocity.y);


		if (rid.velocity.y >= 0)
			rid.gravityScale = Parameters.gravityUp;
		else if (rid.velocity.y < -0.1)
	...
	public void Jump (UnityEngine.EventSystems.BaseEventData data)
	{
		...
		if (enviroment != Enviroment.Water) {
			if (grounded) {
				rid.velocity = new Vector2 (rid.velocity.x, Parameters.jumpHeight);
				// currentLep++;
	...
	public void PushPlayer ()
	{
		...
		if (posHit.x > transform.position.x) {
			...
			if (horizontal >= 0) {
				...
				rid.velocity = new Vector2 (-MaxPush, rid.velocity.y);
	...
	public void OnTriggerEnter2D (Collider2D other)
	{
		if (other.tag == "Water") {
			...
			rid.AddForce (new Vector2 (0, forcePushWhenFall * Mathf.Abs (rid.velocity.y)));
		}
	}
